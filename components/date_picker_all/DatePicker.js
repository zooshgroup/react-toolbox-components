...
import { DATE_PICKER } from 'react-toolbox/src/components/identifiers';
import InjectIconButton from 'react-toolbox/lib/button/IconButton';
...
const factory = (Input, DatePickerDialog) => {
  class DatePicker extends Component {
    static propTypes = {
      ...
      showAllButtonLabel: PropTypes.string,
    ...
    render() {
      const { active, onDismiss, showAllButtonLabel, showAllLabel, // eslint-disable-line
      ...
      let formattedDate;
      if (value === '') {
        formattedDate = this.props.showAllLabel;
      } else {
        date = Object.prototype.toString.call(value) === '[object Date]' ? value : undefined;
        formattedDate = date === undefined ? '' : finalInputFormat(value, locale);
      }
      ...
      return (
        <div data-react-toolbox="date-picker" className={this.props.theme.container}>
          ...
          <DatePickerDialog
            ...
            onShowAllClick={(val, event) => this.handleShowAll(event)}
            showAllLabel={this.props.showAllButtonLabel}
